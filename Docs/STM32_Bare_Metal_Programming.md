...
RCC_BASE (fixed address 0x40023800): used to control internal peripherals and to distribute clocks and reset signals. It can be found in the manual on page 50 under the name Reset and clock control RCC.

GPIOA_BASE (fixed address 0x40020000) refers to the A port GPIO. It can be found in the manual on page 58 and 171.
...
GPIOA_MODER is calculated in relation to GPIOA_BASE (GPIOA_BASE + 0x00). It refers to port mode configuration register and can be found on page 187.

GPIOA_OTYPER is calculated in relation to GPIOA_BASE (GPIOA_BASE + 0x04). It refers to port output type register. It can be found on page 188.
